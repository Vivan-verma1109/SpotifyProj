...
public class PlaylistReader
{
        ...
        FileNotFoundException
    {
        ...
        PlaylistCalculator playlist = new PlaylistCalculator(queue, playlists);
        PlaylistWindow pw = new PlaylistWindow(playlist);
        ...
        FileNotFoundException
    {
            ...
            && playlistCounter < PlaylistCalculator.NUM_PLAYLISTS)
        {
            ...
            if (tokens.length < 8)
            {
                throw new java.text.ParseException(read, playlistCounter);
            }
            ...
            else
            {
                ...
                if (!(minPop >= 0 && maxPop <= 100))
                {
                    throw new DailyMixDataException("bad range of pop");
                }
                if (!(minRock >= 0 && maxRock <= 100))
                {
                    throw new DailyMixDataException("bad range of Rock");
                }
                if (!(minCountry >= 0 && maxCountry <= 100))
                {
                    throw new DailyMixDataException("bad range of Country");
                }
        ...
        FileNotFoundException
    {
        ...
        while (file.hasNextLine())
        {
            ...
            {
                throw new java.text.ParseException(read, tokens.length);
            }
            ...
            if (country < 0 || country > 100)
            {
                throw new DailyMixDataException("bad range of Country");
            }

            Song s = new Song(songName, pop, rock, country, playlist);

            parsedSongs.enqueue(s);

            if (tokens.length < 4)
            {
                throw new java.text.ParseException("length not long enough", tokens.length);
            }
    ...
    {
        return num1 >= MIN_PERCENT && num1 < MAX_PERCENT && num2 >= MIN_PERCENT
            && num2 < MAX_PERCENT && num3 >= MIN_PERCENT && num3 < MAX_PERCENT;
